...
## 2026-09-23 — Cinematique prompt-language layer + StepFun audio house (workflow 12)

- **Cinematique** (vvsvs.pro/cinematique, free/open) adopted as the prompt-PHRASING layer in `creative/eyecandy-catalog.md`: Eyecandy names it → Cinematique phrases it → PERIOD STOCK constrains it → Shot Composer verifies it → wallet renders it. Curated period-filtered vocabulary added (lighting: low-key/chiaroscuro/practicals/short light/kicker/eye light/gobo/color-temperature dramaturgy; composition: framing-within-frame/deep focus/figure-ground/leading lines; editing grammar). PERIOD GATE rule: Cinematique's modern gear citations (ARRI/Vision3) never enter 1959 prompts.
- **StepFun** (api.stepfun.ai, international zone — the .com zone 401s this key) adopted as the AUDIO LAYER: workflow 12 `stepfun.py` (house style; TTS verified shape, ASR/music/image-edit documented as next). Key in `jevmax\.env` as STEPFUN_API_KEY (gitignore-protected + push-overlay-excluded). Pricing: $0.36/10k chars TTS, $1.50/voice clone. **Status: HTTP 402 quota_exceeded on first live call — account has no balance; top up at platform.stepfun.ai billing.** Once funded: `python stepfun.py tts "..." --out line.mp3`. Role: dialogue TTS for --no-audio shots, SEVEN-THREE's voice, music cues, ASR as audio-QC gate. step-image-edit-2 flagged as the bible detail-gap bridge (unverified shape). API key was pasted in chat — rotate from dashboard if that matters.

## 2026-09-23 — Depth conditioning layer (Depth Anything V2 Small)

- **Depth Anything V2 Small** (Apache 2.0, ~25M params) adopted as the depth-estimation step for generation conditioning: keyframe → grayscale depth map → structural conditioning for image-to-video, or continuity check vs Shot Composer previz.
- Pipeline step: `projects/dial-m-for-moon/tools/depth_condition.py` — `python tools/depth_condition.py keyframe.png depth.png`. CPU-only (~30s/frame incl. weight load), local model dir default (no HF download at runtime).
